...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
        ...
        children: [
          ...
          Center(
            child: Column(children: [
              ...
              const Text('Smart Money Manager', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
              const Text('v1.0.0', style: TextStyle(color: AppColors.lightTextSecondary, fontSize: 12)),
            ]),
          ).animate().fadeIn(),
          ...
          _SettingCard(children: [
            _SettingRow(
              ...
              trailing: Switch(
                ...
                onChanged: (v) async {
                  await ref.read(settingsProvider.notifier).update(settings.copyWith(incomeReminderEnabled: v));
                  if (v) {
          ...
          _SettingCard(children: [
            _SettingRow(
              icon: Icons.pin_outlined, label: 'PIN Lock',
              subtitle: settings.pinEnabled ? 'Enabled' : 'Disabled',
              onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const PinSetupScreen())).then((_) => ref.invalidate(settingsProvider)),
            ),
            if (_biometricAvailable)
              _SettingRow(
                ...
                trailing: Switch(
                  ...
                  onChanged: settings.pinEnabled ? (v) => ref.read(settingsProvider.notifier).update(settings.copyWith(biometricEnabled: v)) : null,
                  activeColor: AppColors.primary,
                ),
              ),
          ]),
          ...
          _SettingCard(children: [
            _SettingRow(
              icon: Icons.sms_outlined, label: 'Scan SMS Inbox',
              subtitle: 'Import historical transactions from SMS',
              onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const SmsScannerScreen())),
            ),
          ]),
          ...
          _SettingCard(children: [
            _SettingRow(
              icon: Icons.upload_outlined, label: 'Export Data',
              subtitle: 'Save as JSON file',
              onTap: _exportData,
            ),
            _SettingRow(
              icon: Icons.download_outlined, label: 'Import Data',
          ...
          _SettingCard(children: [
            _SettingRow(icon: Icons.code_outlined, label: 'Built with Flutter', subtitle: 'Open source'),
            _SettingRow(icon: Icons.favorite_outline, label: 'Made with ❤️ by TheDayDreamer17'),
          ]),
  ...
  Future<void> _exportData() async {
    try {
      ...
      final file = File('${dir.path}/smart_money_backup_${DateTime.now().millisecondsSinceEpoch}.json');
      await file.writeAsString(json);
      await Share.shareXFiles([XFile(file.path)], text: 'Smart Money Manager Backup');
    } catch (e) {
    ...
    try {
      ...
      final confirm = await showDialog<bool>(
        ...
        builder: (_) => AlertDialog(
          ...
          content: Text('This will merge data from backup (${data['exported_at']}). Existing data will NOT be deleted.'),
          actions: [
      ...
      final db = DatabaseHelper.instance;
      for (final table in ['accounts', 'categories', 'transactions', 'budgets', 'trips', 'goals', 'subscriptions', 'net_worth_entries']) {
...
class _SectionHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Text(title, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: AppColors.primary, letterSpacing: 0.5)),
  );
}
...
class _SettingCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(children: children.indexed.map((e) {
...
class _SettingRow extends StatelessWidget {
  ...
  const _SettingRow({required this.icon, required this.label, this.subtitle, this.trailing, this.onTap});
  ...
  Widget build(BuildContext context) => ListTile(
    ...
    trailing: trailing ?? (onTap != null ? const Icon(Icons.chevron_right, size: 18, color: AppColors.lightTextSecondary) : null),
    onTap: onTap,
