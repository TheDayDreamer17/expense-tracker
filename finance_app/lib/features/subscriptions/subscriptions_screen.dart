...
class _SubscriptionsScreenState extends ConsumerState<SubscriptionsScreen> {
  ...
  Future<void> _load() async {
    final rows = await DatabaseHelper.instance.query('subscriptions', orderBy: 'next_billing_date ASC');
    if (mounted) setState(() { _subs = rows.map(SubscriptionModel.fromMap).toList(); _loading = false; });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Subscriptions'), actions: [IconButton(icon: const Icon(Icons.add), onPressed: _showSubSheet)]),
      body: _loading
                  ...
                  Container(
                    ...
                    child: Row(children: [
                      ...
                      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        const Text('Monthly Subscriptions', style: TextStyle(color: Colors.white70, fontSize: 13)),
                        Text(CurrencyFormatter.format(_monthlyTotal), style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.w700)),
                        Text('${_subs.length} active subscriptions', style: const TextStyle(color: Colors.white60, fontSize: 12)),
                      ]),
                    ]),
                  ).animate().fadeIn(),
                  ...
                  if (_subs.isEmpty)
                    const Center(child: Padding(
                      ...
                      child: Column(children: [
                        ...
                        SizedBox(height: 12),
                        Text('No subscriptions tracked', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
                      ]),
                    ))
                  else
...
class _SubCard extends StatelessWidget {
  ...
  const _SubCard({required this.sub, required this.onTap, required this.onMarkUsed});

  @override
  Widget build(BuildContext context) {
    final daysUntil = sub.daysUntilBilling;
    final billingColor = daysUntil <= 3 ? AppColors.expense : daysUntil <= 7 ? AppColors.warning : AppColors.income;
    return Card(
        ...
        title: Row(children: [
          ...
          if (sub.isUnused)
            Container(
              ...
              child: Text('${sub.daysSinceLastUsed}d unused', style: const TextStyle(fontSize: 10, color: AppColors.warning, fontWeight: FontWeight.w600)),
            ),
        ]),
        subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(sub.category, style: const TextStyle(fontSize: 12, color: AppColors.lightTextSecondary)),
          Text(
            daysUntil == 0 ? '⚡ Billing today!' : daysUntil < 0 ? 'Overdue' : 'Bills in $daysUntil days',
            style: TextStyle(fontSize: 11, color: billingColor, fontWeight: FontWeight.w600),
          ),
        ]),
        trailing: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.end, children: [
          Text('₹${sub.amount.toStringAsFixed(0)}', style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 15)),
          Text(sub.billingCycle == 'YEARLY' ? '/yr' : '/mo', style: const TextStyle(fontSize: 11, color: AppColors.lightTextSecondary)),
        ]),
        onTap: onTap,
...
class _SubFormSheetState extends State<_SubFormSheet> {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 16),
        Text(widget.sub == null ? 'Add Subscription' : 'Edit Subscription', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
        const SizedBox(height: 12),
        // Quick presets
        SizedBox(height: 40, child: ListView(scrollDirection: Axis.horizontal, children: _presets.map((p) =>
          GestureDetector(
            onTap: () => setState(() { _nameCtrl.text = p['name']!; _icon = p['icon']!; _category = p['category']!; }),
            child: Container(
              ...
              child: Text('${p['icon']} ${p['name']}', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: AppColors.primary)),
            ),
          ),
        ).toList())),
        const SizedBox(height: 12),
        TextField(controller: _nameCtrl, decoration: const InputDecoration(labelText: 'Subscription Name')),
        const SizedBox(height: 12),
        Row(children: [
          Expanded(child: TextField(controller: _amountCtrl, decoration: const InputDecoration(labelText: 'Amount', prefixText: '₹ '), keyboardType: TextInputType.number)),
          const SizedBox(width: 12),
        ...
        InkWell(
          onTap: () async {
            final d = await showDatePicker(context: context, initialDate: _nextDate, firstDate: DateTime.now(), lastDate: DateTime(2100));
            if (d != null) setState(() => _nextDate = d);
          },
          child: InputDecorator(decoration: const InputDecoration(labelText: 'Next Billing Date'),
        ...
        SizedBox(width: double.infinity, child: ElevatedButton(onPressed: _saving ? null : _save,
            child: Text(widget.sub == null ? 'Add Subscription' : 'Update'))),
      ])),
    );
  }
  ...
  Future<void> _save() async {
    ...
    await DatabaseHelper.instance.insert('subscriptions', {
      ...
      'name': _nameCtrl.text.trim(), 'amount': double.tryParse(_amountCtrl.text) ?? 0,
      'billing_cycle': _cycle, 'next_billing_date': _nextDate.millisecondsSinceEpoch,
      'last_used_date': widget.sub?.lastUsedDate?.millisecondsSinceEpoch,
      'icon': _icon, 'category': _category,
      'created_at': widget.sub?.createdAt.millisecondsSinceEpoch ?? DateTime.now().millisecondsSinceEpoch,
    });
    widget.onSaved();
