...
class _SmsScannerScreenState extends ConsumerState<SmsScannerScreen> {
  ...
  Future<void> _scan() async {
    setState(() { _scanning = true; _done = false; _results = []; _selected = {}; });
    final parsed = await SmsService.instance.scanInbox(months: _months);

    // Check for duplicates in DB
    final db = DatabaseHelper.instance;
    final existingRaw = await db.query('transactions', where: 'is_sms_imported = 1');
    final existingAmounts = existingRaw.map((r) => (r['amount'] as num).toDouble()).toSet();

    final results = <_ScanResult>[];
    for (final p in parsed) {
      final isDup = existingAmounts.contains(p.amount);
      results.add(_ScanResult(parsed: p, isDuplicate: isDup));
    }

    setState(() {
      _results = results;
      _selected = results.asMap().entries.where((e) => !e.value.isDuplicate).map((e) => e.key).toSet();
      _scanning = false;
      _done = true;
    });
  }
        ...
        children: [
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Row(children: [
                  const Text('Scan last: ', style: TextStyle(fontWeight: FontWeight.w600)),
          ...
          if (_done && _results.isEmpty)
            const Expanded(child: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              SizedBox(height: 12),
              Text('No transaction SMS found', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
            ])))
          else if (_results.isNotEmpty) ...[
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 16),
              child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Text('${_results.length} found · ${_selected.length} selected', style: const TextStyle(fontWeight: FontWeight.w600)),
                TextButton(
                  onPressed: () => setState(() {
                ...
                itemBuilder: (_, i) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      leading: Checkbox(
                        value: isSelected,
                        onChanged: r.isDuplicate ? null : (v) => setState(() => v! ? _selected.add(i) : _selected.remove(i)),
                        activeColor: AppColors.primary,
                      ),
                      title: Row(children: [
                        Text(CurrencyFormatter.format(r.parsed.amount),
                            style: TextStyle(fontWeight: FontWeight.w700,
                                color: r.parsed.type == 'EXPENSE' ? AppColors.expense : AppColors.income)),
                        const SizedBox(width: 8),
                        if (r.isDuplicate) Container(
                      ...
                      subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        if (r.parsed.merchant != null) Text(r.parsed.merchant!, style: const TextStyle(fontSize: 12)),
                        Text(_catName(r.parsed.suggestedCategory), style: const TextStyle(fontSize: 11, color: AppColors.primary)),
                      ]),
                      trailing: Text(r.parsed.type == 'EXPENSE' ? '🔴' : '🟢', style: const TextStyle(fontSize: 18)),
                    ),
  ...
  String _catName(String id) {
    const map = {'cat_food': '🍕 Food', 'cat_grocery': '🛒 Groceries', 'cat_transport': '🚗 Transport',
      'cat_shopping': '🛍️ Shopping', 'cat_entertainment': '🎬 Entertainment', 'cat_health': '💊 Health',
      'cat_utilities': '⚡ Utilities', 'cat_telecom': '📱 Telecom', 'cat_education': '🎓 Education',
      'cat_other_exp': '❓ Other'};
    return map[id] ?? '❓ Other';
  }
}
