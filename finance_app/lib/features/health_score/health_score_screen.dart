...
class _HealthScoreScreenState extends ConsumerState<HealthScoreScreen> {
  ...
  Future<void> _compute() async {
    ...
    if (monthMap.isNotEmpty) {
      avgIncome = monthMap.values
              .map((m) => m['INCOME'] ?? 0)
              .fold(0.0, (a, b) => a + b) /
          monthMap.length;
      avgExpense = monthMap.values
              .map((m) => m['EXPENSE'] ?? 0)
              .fold(0.0, (a, b) => a + b) /
          monthMap.length;
    }
    ...
    final nwRows = await db.rawQuery(
        'SELECT entry_type, SUM(amount) as total FROM (SELECT name, entry_type, amount FROM net_worth_entries GROUP BY name ORDER BY date DESC) GROUP BY entry_type');
    double totalAssets = 0, totalLiabilities = 0;
    for (final r in nwRows) {
      if (r['entry_type'] == 'ASSET')
        totalAssets = (r['total'] as num).toDouble();
      if (r['entry_type'] == 'LIABILITY')
        totalLiabilities = (r['total'] as num).toDouble();
    }
    ...
    final restMonthTx = await db.rawQuery('''
      ...
      WHERE type = 'EXPENSE' AND date >= ?
        AND CAST(strftime('%d', datetime(date/1000,'unixepoch')) AS INTEGER) > 7
    ''', [thisMonthStart]);
    final salaryWeekSpend =
        (salaryWeekTx.first['total'] as num?)?.toDouble() ?? 0;
    final restMonthSpend =
        (restMonthTx.first['total'] as num?)?.toDouble() ?? 0;
    final salaryWeekDailyAvg = salaryWeekSpend / 7;
    final restMonthDailyAvg = restMonthSpend / 23;
    final salarySpike =
        restMonthDailyAvg > 0 ? salaryWeekDailyAvg / restMonthDailyAvg : 0.0;

    // ─── Score computation ─────────────────────────────────────────────────
    // 1. Savings rate (20 pts): (income - expense) / income
    final savingsRate =
        thisIncome > 0 ? (thisIncome - thisExpense) / thisIncome : 0.0;
    ...
    final efRatio = emergencyTarget > 0
        ? (totalAssets / emergencyTarget).clamp(0, 1.0)
        : 0.0;
    final efScore = (efRatio * 20).round();

    // 3. Debt ratio (20 pts): liabilities / assets
    final debtRatio =
        totalAssets > 0 ? (totalLiabilities / totalAssets).clamp(0, 1.0) : 1.0;
    final debtScore = ((1 - debtRatio) * 20).round();

    // 4. Lifestyle inflation (15 pts): expense vs 3-month avg
    final lifestyleChange =
        avgExpense > 0 ? ((thisExpense - avgExpense) / avgExpense) : 0.0;
    final lifestyleScore = lifestyleChange > 0.2
        ? 0
        : lifestyleChange > 0
            ? 8
            : 15;
    ...
    final investmentScore = nwEntries.length >= 3
        ? 15
        : nwEntries.length >= 1
            ? 8
            : 0;

    final total = savingsScore +
        efScore +
        debtScore +
        lifestyleScore +
        subScore +
    ...
    if (savingsRate < 0.1)
      insights.add("💸 You're saving less than 10% of your income this month.");
    if (savingsRate >= 0.2)
      insights.add(
          "🎉 Great! You saved ${(savingsRate * 100).toInt()}% of your income this month.");
    if (efRatio < 0.5)
      insights.add(
          "⚠️ Emergency fund is below 1.5 months of expenses. Consider building it up.");
    if (lifestyleChange > 0.2)
      insights.add(
          "📈 Spending increased ${(lifestyleChange * 100).toInt()}% vs your 3-month average.");
    if (weekendRatio > 2)
      insights.add(
          "🍻 Weekend spending is ${weekendRatio.toStringAsFixed(1)}× your weekday average.");
    if (salarySpike > 1.5)
      insights.add(
          "📅 Salary week spending is ${(salarySpike * 100).toInt()}% higher than rest of month.");
    if (unusedCount > 0)
      insights.add(
          "🔄 $unusedCount subscription(s) unused for 14+ days — consider cancelling.");
    if (debtRatio > 0.5)
      insights.add(
          "🔴 Debt ratio is high (${(debtRatio * 100).toInt()}% of assets). Focus on paying down liabilities.");

    if (mounted) {
      setState(() {
        _score = total.clamp(0, 100);
        _components = {
          'Savings Rate': _ScoreComponent(
              savingsScore, 20, '💰', 'Save at least 20% of income'),
          'Emergency Fund':
              _ScoreComponent(efScore, 20, '🛡', '3+ months of expenses saved'),
          'Debt Ratio': _ScoreComponent(
              debtScore, 20, '🏦', 'Keep liabilities below 50% of assets'),
          'Lifestyle Inflation': _ScoreComponent(lifestyleScore, 15, '📊',
              'Avoid spending spikes month over month'),
          'Subscriptions':
              _ScoreComponent(subScore, 10, '🔄', 'No unused subscriptions'),
          'Investments': _ScoreComponent(
              investmentScore, 15, '📈', 'Track your investments regularly'),
        };
        _insights = insights;
        _loading = false;
      });
          ...
          : RefreshIndicator(
              ...
              child: ListView(
                ...
                children: [
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        gradient: SweepGradient(
                          ...
                          colors: [
                            _scoreColor(_score),
                            _scoreColor(_score).withOpacity(0.3)
                          ],
                        ),
                        boxShadow: [
                          BoxShadow(
                              color: _scoreColor(_score).withOpacity(0.3),
                              blurRadius: 24,
                              spreadRadius: 4)
                        ],
                      ),
                  ...
                  ..._components.entries
                      .map((e) => _ComponentRow(name: e.key, comp: e.value))
                      .toList(),
